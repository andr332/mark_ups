  ...
  <div>
    ...
    <div class="outer-container">
      <form class="inner-container" v-on:submit.prevent="submit()">
        ...
        <label class="field-label">New Email <label class="field-label-required">*</label></label>
        <input class="input-field" type="email" v-model="email">
        <button class="btn-submit" v-on:click.prevent="submit()">Change Email</button>
      </form>
    </div>
    <Footer></Footer>
  </div>
...
<script>
  import {changeEmail, clearUserCache} from "../services/authService";
    ...
    methods: {
      async submit() {
        try {
          ...
          await changeEmail(this.email);
          await clearUserCache();
          this.showEmailConfirmationMsg();
          this.redirectToAccount();
        } catch (err) {
          if (err.code === 'AliasExistsException') {
            this.showEmailAlreadyInUseMsg();
          } else if (err.code === 'InvalidEmailException') {
            this.showInvalidEmailMsg();
          } else if (err.code === 'EmailNotProvided') {
            // do nothing
          } else if (err.code === 'CurrentEmailProvidedException') {
            this.showCurrentEmailProvidedMsg();
          } else {
            this.showErrorMsg();
          }
        } finally {
      ...
      validateForm() {
        if (!this.email) {
          throw {
            code: 'EmailNotProvided'
          };
        }

        if (this.email === this.userEmail) {
          throw {
            code: 'CurrentEmailProvidedException'
          };
        }
